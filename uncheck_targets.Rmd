    ...
    tibble(
      ...
      mod = list(try(inla(
            formula = formula_inla(
              ...
              plot_effect = ifelse(biomass_trait == "bm", FALSE, TRUE)
              ),
            control.compute = list(dic = TRUE, waic = TRUE, cpo = TRUE,
              return.marginals=TRUE, return.marginals.predictor=TRUE),
...
```{r, results = "hide"}
tar_load(p_obs_fit_tot)
ggsave(
  filename = "p_obs_fit_tot.png",
  plot = p_obs_fit_tot,
...
```{r, results = "hide"}
tar_load(p_obs_fit_sp_tot)
ggsave(
  filename = "p_obs_fit_sp_tot.png",
  plot = p_obs_fit_sp_tot,
  path = here("paper", "graphics"),
  scale = 2.4,
  width = 90,
  height = 90 * 1,
