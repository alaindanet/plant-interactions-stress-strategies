...
```{r, eval = FALSE}
p_mechanistic <- plot_grid(
  p_effect_trait + theme(legend.position = "right"),
  print(p_div_effects),
  nrow = 2,
  rel_heights = c(1, 1),
  labels = "auto"
...
env_raw_data %>%
  pivot_longer(
    c(soil.moisture, temperature, PAR),
    names_to = "variable",
    values_to = "values") %>%
mutate(
  variable = response_replacement()[variable]
  ) %>%
ggplot(aes(y = values, x = as.factor(duration_m), color = as.factor(watering), linetype = as.factor(ms))) +
    geom_boxplot() +
...
list(
  tar_target(p_bm_pp_ci,
    inta_bm_pred_ci %>%
      ...
      ggplot(aes(y = mean, x = duration_m + 5, color = watering, shape = com)) +
      geom_point(size = 3) +
      geom_line() +
  ...
  tar_target(p_surv_pp_ci,
    inta_surv_pred_ci %>%
      filter(species == "anthyllis", level == "community") %>%
      mutate(com = recode_factor(com, "Mono" = "Monospecific", "Poly" = "Mixture")) %>%
      ggplot(aes(y = mean, x = duration_m + 5, color = watering, shape = com)) +
      geom_point(size = 3) +
      geom_line() +
      geom_ribbon(aes(ymin = lower, ymax = upper, fill = watering), alpha = .1,
...
list(
  ...
  tar_target(p_other_leaf,
    ...
    labs(x = "Standardized coefficients", color = "Trait") +
...
```{r, eval=FALSE}
tar_load(bm_inla_rand_effects)
tar_load(bm_inla)
get_hpdmarginal_inla(bm_inla$mod[[4]], type = "rand")

tar_load(c(var_fitted, var_rand_effect))
var_fitted %>%
...
tar_target(p_pred_surv,
  surv_data_modelling %>%
    ...
    ggplot(aes(y = survival, x = duration_m + 5, shape = com, color = watering)) +
    geom_jitter(alpha = .2) +
    geom_line(data = inla_surv_pred, aes(y = mean)) +
    geom_point(data = inla_surv_pred, aes(y = mean)) +
...
mutate(
  ...
  metric = factor(metric, levels = c("Net", "Complementarity", "Selection")),
  ms = recode_factor(ms, "Open" = "Open", "Patch" = "Nurse patch")
)

p_div_effects <- div_effects_lg %>%
  ...
  labs(
    ...
    y = "Diversity effect",
    color = "Watering") +
  scale_y_break(c(0.2, 1.5), scale = "free") +
  theme_bw()

leg <- get_legend(p_div_effects + theme(legend.position = "bottom"))

p_div_effects_ok <- plot_grid(
  print(p_div_effects + theme(legend.position = "none")),
  leg,
  nrow = 2, rel_heights = c(1,.1)
)

ggsave(
  filename = "p_div_effects.png",
  plot = p_div_effects_ok,
  path = here("paper", "graphics"),
  scale = 1.8,
  width = 90,
  height = 90 * .6,
...
```{r}
term2rm <- c("(Intercept)", "ter2", "ter3", "ter4", "speciesdorycnium", "speciespistachia")
```
...
p_int_global_surv <- pp_int_global %>%
  ...
  ggplot(aes(y = inta, x = duration_m + 6, color = watering, shape = com, fill = watering), alpha = 1) +
  geom_line() +
  geom_point(size = 3) +
...
```{r}
tar_load(inla_surv_odd)
p_surv <- plot_inla_fixed_effect(inla_surv_odd,
  color_var = NULL,
  intercept = 0,
  term_to_rm = term2rm) +
  labs(x = "Survival odd-ratios (exp(beta) - 1)") +
...
```{r}
tar_load(c(surv_data_modelling, inla_surv_pred, inla_surv_sp_pred, p_pred_surv))
...
```{r, fig.cap = "Biomass"}
tar_load(c(bm_inla_pred, bm_inla_sp_pred, bm_inla_effects))
p_bm <- plot_inla_fixed_effect(
  bm_inla_effects %>% filter(response == "bm"),
  color_var = NULL, intercept = 0, term_to_rm = term2rm) +
  labs(x = "Slope coefficients for total biomass") +
  theme(axis.title.y = element_blank())
...
tar_load(c(leaf_inla_sp_pred, leaf_trait_modelling, leaf_inla_effects_scaled))

p_effect_trait <- plot_inla_fixed_effect(
