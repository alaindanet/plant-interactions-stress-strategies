...
library(zoo)
library(ecocom)
tar_option_set(packages = c("tidyverse", "here", "magrittr", "lubridate", "INLA", "zoo", "ecocom", "cowplot", "tarchetypes"))
...
# watering contrary to the other that were done just after
#,
#  november %>%
#    rename(notes = ...11)
```
...
tot_pred <- get_pred_data(x = surv_data_modelling,
    predictor = c("com", "ms", "watering", "species", "duration_m", "ter",
      "label"))
ter_pred <- get_pred_data(
  x = surv_data_modelling,
  predictor = c("com", "ms", "watering", "species", "duration_m", "ter")
  ) %>%
select(-label) %>%
left_join(distinct(surv_data_modelling, ter, label))
unique(ter_pred$label)
summary(ter_pred)
head(ter_pred)
simple_pred <- get_pred_data(x = surv_data_modelling,
    predictor = c("com", "ms", "watering", "species", "duration_m"))
nrow(tot_pred) / nrow(simple_pred)
tar_load(pred_var)
pred_var[pred_var != "label"]
```


```{targets data_modelling, eval = TRUE}
list(
  tar_target(n_sampling, 100),
  ...
  tar_target(pred_var, c("com", "ms", "watering", "species", "duration_m", "ter", "label")),
  tar_target(pred_surv_data, get_pred_data(x = surv_data_modelling,
    predictor = c("com", "ms", "watering", "species", "duration_m"))),
  tar_target(pred_surv_data_exp, get_pred_data(
  x = surv_data_modelling,
  predictor = pred_var[pred_var != "label"]
  ) %>%
select(-label) %>%
left_join(distinct(surv_data_modelling, ter, label))),
...
list(
  tar_target(biomass_trait, c("d", "h", "bm")),
  tar_target(bm_inla,
    tibble(
...
list(
  ...
  tar_target(bm_inla_pred_samp,
    ...
    mutate(samp = map(mod,
        ~inla_sampling_from_pred(model = .x, pred_data = pred_surv_data, nsamp = n_sampling)
      )
      ) %>%
    select(-mod)
  ),
...
```{r}
tar_load(c(inla_surv_odd, term2rm))
plot_inla_fixed_effect(inla_surv_odd,
      color_var = NULL,
      color_point = wet_dry_color()["Not Watered"],
      intercept = 0,
      term_to_rm = term2rm) +
...
```{targets fig3_alt, tar_simple = TRUE}
plot_inla_fixed_effect(
  leaf_inla_effects_scaled %>%
...
list(
  tar_target(bm_inla_sp_pred_sampling, {
    ...
    mutate(
      sampling = map(fitted,
        ~tibble(value = inla.rmarginal(n_sampling, .x), rep = 1:length(value))
      )
      ) %>%
    select(-fitted) %>%
    unnest(sampling)
...
library(tarchetypes)
list(
  tar_render(manuscript, here("paper", "main_text.Rmd"))
)
...
```{r}
# Do not work in a pipe bc of external pointer of raster
library(terra)
#install.packages("tidyterra")
library(tidyterra)
library(ggspatial)
r <- rast(here::here("data-raw/Global-AI_ET0_v3_annual/ai_v3_yr.tif"))
crs(r)
```
...
library(sp)
# Site coordinates:
# 37°57’33“N, 1°00’17”W
lat <- "37d57m33sN"
long <- "1d00m17sW"
lat_d <- char2dms(lat, chd='d', chm='m', chs='s') %>% as.numeric()
long_d <- char2dms(long, chd='d', chm='m', chs='s') %>% as.numeric()
longlat <- cbind(longitude = long_d, latitude = lat_d)
site_pt <- vect(longlat)
crs(site_pt) <- crs(r)
murcia_pt <- vect(cbind(-1.130278, 37.986111))
pts_df <- tibble(
  name = c("Murcia", "Study site"),
  long = c(-1.130278, long_d),
  lat = c(37.986111, lat_d),

)
pts <- vect(as_data_frame(pts_df), geom = c("long", "lat"), keepgeom = TRUE)
crs(pts) <- crs(r)
```
...
plot(r)
points(site_pt)
bounding_box <- ext(-9.39288367353, 3.03948408368, 35.946850084, 43.7483377142)
r1 <- crop(r, bounding_box)
x <- subst(r1, 0, NA)
x1 <- disagg(x, 5, method="bilinear")
values(x)  <- round(values(x) / 10000, 2)
values(x1)  <- round(values(x1) / 10000, 2)
```
...
sum(is.nan(values(x1)))
sum(!is.nan(values(x1)))
nan_x1 <- is.nan(values(x1))
#x_c <- app(x1, fun=get_ai_class)
x_c <- rast(ncol=ncol(x1), nrow=nrow(x1))
x_c <- x1
values(x_c) <- NA
values(x_c)[!nan_x1] <- get_ai_class(values(x1)[!nan_x1])
...
writeRaster(
  x_c,
  filename = here("data", "ai_crop_class.tif"),
  overwrite = TRUE)
writeRaster(
  x1,
  filename = here("data", "ai_crop.tif"),
  overwrite = TRUE)
site_coord <- longlat
save(site_coord, file = here("data", "site_coord"))
```


```{r}
p <- ggplot() +
  geom_spatraster(data = x_c) +
  scale_fill_manual(
    name = "Aridity index",
    values = col_fao,
    limits = names(col_fao),
    na.value = NA) +
  geom_spatvector(
         data = pts,
         ...
         size = 1,
         alpha = 1, vjust = 1, hjust = -0.25
       ) +
     scale_color_manual(values = c("Murcia" = "white", "Study site" = "black")) +
    theme_map() +
    theme(
...
```{r}
library(viridis)
p_aridity_index_map <- ggplot() +
  geom_spatraster(data = x1) +
  scale_fill_viridis(
    name = "Aridity\nindex",
    na.value = NA,
    option = "turbo",
...
```{r}
extract(x1, pts, fun = NULL, method = "simple")
```


```{r}
library(lubridate)
# From https://spei.csic.es/map/maps.html#months=4#month=10#year=2016
spei <- read_csv("~/Téléchargements/SPEI_37.75_-1.25.csv")
```

```{r}
spei_long <- spei %>%
  mutate(date = my(DATA)) %>%
  filter(date > "1951-01-01") %>%
  select(date, SPEI_1, SPEI_3, SPEI_6, SPEI_12, SPEI_24) %>%
  pivot_longer(-date, names_to = "duration", values_to = "spei") %>%
  mutate(month = as.integer(str_extract(duration, "\\d{1,2}")))
max(spei_long$date)
min(spei_long$date)
spei_filtered <- spei_long %>%
  filter(date >= "2015-11-01" & date <= "2016-11-01")
spei_filtered %>%
  group_by(month) %>%
  summarise(
    avg = mean(spei),
...
## Highlight dates from the experimental set-up to final measurement
experiment_date_range <- tibble(
  x1 = ymd("2015-11-01"), x2 = ymd("2016-11-01"),
  y1 = -Inf, y2 = +Inf
)
paste0(unique(spei_long$month), " month")
p_spei <- spei_long %>%
  filter(month %in% c(3, 6, 12)) %>%
  mutate(month = factor(month, labels = paste0(unique(month), " months"))) %>%
  ggplot(aes(x = date, y = spei)) +
  geom_line(color = "darkblue") +
  geom_area(fill = "lightblue") +
  geom_rect(
    data = experiment_date_range,
    inherit.aes = FALSE,
  ...
  labs(x = "Date", y = "SPEI", color = "Month\nspan") +
  ylim(-2, 2) +
  theme_bw()
ggsave_multiple(
  paste0("p_spei", c(".png", ".pdf")),
  plot = p_spei + guides(
      color = guide_legend(
        keywidth = unit(.3, "cm"),
        keyheight = unit(.2, "cm"),
...
```{r}
tar_load(c(leaf_inla_effects_scaled, term2rm))
ggsave_multiple(
  paste0("fig3_alt", c(".png", ".pdf")),
  plot = tar_read(fig3_alt),
  path = here("paper", "graphics"),
  scale = 2.4,
  width = 80,
  height = 80 * .5,
...
ggsave(
  ...
  plot = plot_grid(tar_read(p_bm), tar_read(p_int_global_bm)),
  path = here("paper", "graphics"),
  scale = 2.4,
  width = 90,
  height = 90 * .6,
...
ggsave(
  filename = "p_bm_int.pdf",
  plot = plot_grid(tar_read(p_surv), tar_read(p_int_global_surv)),
  path = here("paper", "graphics"),
  scale = 2.4,
  width = 90,
  height = 90 * .6,
...
```{r}
tar_load(c(p_pred_surv, p_surv, p_pred_bm, p_bm))
...
```{r}
tar_load(c(env_plot, date_nb_month))
not_march <- date_nb_month$date[date_nb_month$date != "2016-03-01"]

ggsave_multiple(
  paste0("p_env_raw_data", c(".png", ".pdf")),
  plot = env_plot +
    scale_x_date(date_labels = "%b", breaks = not_march) +
    theme(legend.position = "bottom"),
  path = here("paper", "graphics"),
  scale = 2.4,
  dpi = 400,
...
p_bm_d_effect <- plot_inla_fixed_effect(ti,
  ...
  scale_color_manual(values = setNames(species_color(), NULL)) +
...
tar_load(bm_inla_pred_samp)
tar_load(inta_bm_pred)
bm_inla_pred_samp %>%
      filter(response == "bm") %>%
      unnest(samp)
```
