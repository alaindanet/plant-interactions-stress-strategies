...
x <- subst(r1, 0, NA)
plot(x)
values(x)  <- values(x) / 10000
plot(x, col = grDevices::heat.colors(50))
points(site_pt, col = "red")
...
x_c <- x
values(x_c) <- get_ai_class(values(x_c))
...
#p +
#  coord_sf(xlim = c(-1.5, -0.5), ylim = c(37.7, 38.2), expand = FALSE)
ggsave_multiple(
  paste0("map_aridity", c(".png", ".pdf")),
  plot = p +
    coord_sf(
...
points(pts[1])
points(pts[2])
bounding_box_site <- ext(-1.5, -0.5, 37.7, 38.2)
r1_site <- crop(r, bounding_box_site)
x_site <- subst(r1_site, 0, NA)
plot(x_site)
values(x_site) <- values(x_site) / 10000
```
