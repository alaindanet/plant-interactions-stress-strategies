...
broom::tidy(biomass_modelling$log_mod_bm_tot_h) %>%
  kable(caption = mod_cap, label = "tab:pred_bm")
```
...
#tar_load(c(p_obs_fit_surv, p_obs_fit_leaf, p_obs_fit))
tar_read(p_obs_fit_tot)
```
...
#tar_load(c(p_obs_fit_surv_sp, p_obs_fit_sp_leaf, p_obs_fit_sp))
tar_read(p_obs_fit_sp_tot)
```
...
```{r}
library(INLA)
ti <- map_dbl(leaf_inla$mod[[1]]$marginals.fitted.values,
            ~var(inla.rmarginal(1000, .x)))
gaussian_inla_var_fitted
rbind(leaf_inla) %>%
      mutate(
        var_pred = map(mod, function(y) {
...
plot_grid(print(surv_base), print(surv_species), labels = "auto")
```
