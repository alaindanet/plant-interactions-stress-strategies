...
purple. SPEI is expressed in standard deviation. Data were extracted from
...
Model coefficients of the statistical model on vegetative height (yellow), total
biomass (orange), and basal diameter (blue). Points represent the mean of the
posterior distribution of coefficients; Thin, medium and large bars represent
the credible interval at 95\\%, 90\\% and 80\\%. The model is the same as for Fig.
2b, therefore the coefficients for total biomass are exactly the same. We note
that using either the estimated total biomass or the basal diameter (used with
vegetative height to estimate total biomass) give similar results, meaning
that our results do not depend on the chosen variable." %>%
str_replace_all("\\s+", " ")
```
