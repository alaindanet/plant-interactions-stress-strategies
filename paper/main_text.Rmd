---
title: "Community diversity and abiotic stress interact to drive plant-plant interactions"
...
ggsave(
  filename = "p_pred_effect_surv.png",
  plot = p_pred_effect_surv,
  path = here("paper", "graphics"),
  scale = 2.4,
  width = 90,
...
treatments affects differently the species (Fig. \@ref(fig:sbm)b, see effects in
Fig. SXX). For example, *P. lentiscus* had the highest increase in survival in
the watering treatment [Probably add more examples].
...
knitr::include_graphics(here("paper", "graphics", "p_pred_effect.png"))
```
...
p_effect_trait <- plot_inla_fixed_effect(leaf_inla_effects_scaled) +
  labs(x = "Standardizing coefficients", color = "Trait") +
...
p_pred_leaf <- leaf_trait_modelling %>%
  ...
  group_by(ms, com, watering, trait, species, plot) %>%
  summarise(n = sum(!is.na(values)), values = mean(values, na.rm = TRUE)) %>%
  filter(n >= 3) %>%
  ggplot(aes(y = values, x = ms, color = com, shape = watering)) +
  geom_jitter(alpha = .2) +
  geom_point(data = leaf_inla_sp_pred %>% rename(trait = response), aes(y = mean), size = 2) +
...
```{r}
lt_cap <- paste0("Slope of the statical models at the community level (a) and
  prectictions at the species level (b) for Leaf Dry Matter Content (LDMC) and
  Specific Leaf Area (SLA). (a) Slope coefficients are standardized. Points
  represent the mean of the posterior distribution of coefficients; Thin, medium
  and large bars represent the credible interval at 95\\%, 90\\% and 80\\%. (b) predictions of the model at the species level (See methods), background points display the data averaged by plot (N = 12).") %>%
str_replace_all("\\s+", " ")
```


```{r lt, fig.cap=lt_cap}
knitr::include_graphics(here("paper", "graphics", "p_leaf.png"))
```
...
respectively, Fig. \@ref(fig:lt)a), meaning that saplings develop a more "resource exploitive"
...
community diversity affected leaf traits. We further found variations across
species (Fig. \@ref(fig:lt)b) [TO COMPLETE].
...
knitr::include_graphics(here("paper", "graphics", "p_int_ok.png"))
```
...
This result can be explain by the possible increase in niche complementary
[Could it be tested with @loreau_partitioning_2001 ?] (i.e. decrease in niche
overlap) among saplings in diverse communities, which in turn decrease
competition for resource exploitation[@cardinale_species_2002] and increase the
benefits of facilitation. [May be something about coexistence theory]
...
knitr::include_graphics(here("paper", "graphics", "competition_stress_sgh.png"))
```
...
to produce strong anti-germination compounds but was not reported affect
...
increase of water input. LDMC and SLA average values did not vary according to
the diversity of the communities. However, theory predicts that diversity should
affect more trait overlap than average values [@macarthur_limiting_1967] [Did
not have time to run the null models to test it as in @danet_indirect_2018].

# Conclusion & perspectives

Our study reports that nurse-beneficiary net interaction are modulated by a
strong interaction between the level of abiotic stress and the diversity of the
beneficiary community. We suggest that optimal facilitative effects are
achieved when niche complementary among beneficiary species is high, i.e.
competition among beneficiary is minimal. Further investigations focusing on how
diversity of beneficiary communities modulate nurse-beneficiary interactions can
be a path forward a better understanding of the interplay between competition
and facilitation, and to better connect facilitation theory with the
Biodiversity-Ecosystem Functioning concept [@wright_overlooked_2017].
