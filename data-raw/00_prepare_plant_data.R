...
january_surv <- january_tot %>%
  rename(notes = e) %>%
  mutate(notes =
    ifelse(notes %in% c(1, 2), ifelse(notes == 1, "bad", "very_bad"), "NA")
  )
january_surv2 <- january_surv %>%
  mutate(survival = 1, broken = 0, missing = 0, hm = NA, hm = as.numeric(hm)) %>%
  select(-f)
...
march %>%
  filter(h1 == "Dead")
unique(c(march$h1, march$h2, march$h3))
# Dead are noticed "Dead" and broken are
#noticed "C"
...
march_sp <- march %>%
  ...
  mutate(ind = str_replace(ind, "sp", ""))
## height table
march_h <- march %>% select(-sp1, -sp2, -sp3, -d1, -d2, -d3, -hm1, -hm2, -hm3)
%>% gather(h1, h2, h3, key = "ind", value = "h") %>% mutate(ind =
  str_replace(ind, "h", ""))
## maximum height table
march_hm <- march %>% select(-sp1, -sp2, -sp3, -d1, -d2, -d3, -h1, -h2, -h3) %>%
  gather(hm1, hm2, hm3, key = "ind", value = "hm") %>% mutate(ind =
    str_replace(ind, "hm", ""))
## diameter table
march_d <- march %>% select(-sp1, -sp2, -sp3, -h1, -h2, -h3, -hm1, -hm2, -hm3)
%>% gather(d1, d2, d3, key = "ind", value = "d") %>% mutate(ind =
  str_replace(ind, "d", ""))
## full join
march_tot <- full_join(march_sp, march_h, by = var_joined_by) %>% full_join(.,
  march_d, by = var_joined_by) %>% full_join(., march_hm, by = var_joined_by)
...
mar_surv %>%
  filter(eh == "D", ed == "D", ehm == "D") %>%
  nrow(.)
mar_surv %>%
  filter(eh == "D") %>%
  nrow(.)
mar_surv %>% filter(ehm == "D") %>% nrow(.)
mar_surv %>% filter(ed == "D") %>% nrow(.)

# Missing registered death in diameter columns
...
# Transform data
var_joined_by <- c("ter", "plot", "label", "ms", "com", "watering", "ind", "date") #, "notes" not in june dataset
## species table
june_sp <- june %>%
...
## full join
june_tot <-
  full_join(june_sp, june_h, by = var_joined_by) %>%
...
sort_survival <- function(x, dead = "Dead", casse = "C", missing = "M"){
  ifelse(x %in% c(dead, casse, missing),
    ifelse(x == dead, "D", ifelse(x == missing, "M", "C")),
    1)
}
june_surv <- june_tot %>%
  mutate(
...
june_surv %>%
  filter(eh != "D", ehm == "D")
# One is recorded C in h and D in hm, should be recorded in C in hm

## Broken recording
june_surv %>%
  filter(eh == "C", ed == "C", ehm == "C") %>%
  nrow(.)
june_surv %>%
  filter(eh == "C") %>%
  nrow(.)
...
june_surv %>%
  filter(eh == "C", ehm != "C") ## One miss!
june_surv[which(june_surv$eh == "C" & june_surv$ehm != "C"), c("hm", "ehm")] <- c("C", "C")

## Missing recording
june_surv %>%
  filter(eh == "M" | ed == "M" | ehm == "M")
# En 833, replace M by D
june_surv[which(june_surv$label == 833 & june_surv$ind == "1"), c("h", "hm",
  "eh", "ehm")] <- rep("D", 4)
# En 445 et 938, replace D by M
june_surv[which(june_surv$label %in% c(445, 938) & june_surv$ind == "1"),
  c("h", "hm", "eh", "ehm")] <- rep("M", 8)
# en 803, replace NA by M
june_surv[which(june_surv$label == 803 & june_surv$ind == "3"), c("hm",
  "ehm")] <- rep("M", 2)
# replace NA to 3 in 803
june_surv[which(june_surv$label == 803 & june_surv$ind == "3"), c("ter")] <- 3

replace_numerise <- function(x,
  pattern = "Dead|C",
  replacement = "NA",
  dec = ",") {
  temp <- str_replace(x, pattern, replacement)
...
june_surv2 <- june_surv %>%
  ...
  mutate(
    broken = ifelse(survival == "C", 1, 0),
    # Separate natural death from accidental breaking
    survival = ifelse(survival == "D", 0, 1),
...
# Transform data
var_joined_by <- c("ter", "plot", "label", "ms", "com", "watering", "ind",
  "date") #, "notes" not in november dataset
...
november_surv %>%
  filter(ed == "D") %>%
  nrow(.)
# Dead has not been registered as dead in diameter columns

# Check that dead recorded are the same in h and hm: Which is missing ?
november_surv %>%
  filter(eh != "D", ehm == "D")
# Status not recorded in hm and ehm,
...
november_surv %>%
  filter(eh == "C", ehm != "C") ## One miss!
november_surv[which(november_surv$eh == "C" & november_surv$ehm != "C"),
  c("hm", "ehm")] <- c("C", "C")

## Missing recording
november_surv %>%
  filter(eh == "M" | ed == "M" | ehm == "M")
november_surv[which(november_surv$eh == "M" | november_surv$ed == "M" |
  november_surv$ehm == "M"), c("h", "hm", "eh", "ehm")] <- rep("M", 55*4)
...
november_surv %>%
  filter(h == "RS", e == "resprout")
november_surv %>%
  filter(h == "RS" | e == "resprout") # Let put resprout status in notes
november_surv[which(november_surv$h == "RS"), "e"] <- rep("resprout", 5)
### NF
november_surv %>%
  filter(h == "NF", e == "NF")
november_surv %>%
  filter(h == "NF" | e == "NF") # Let put NF from h to notes
november_surv %>%
filter(h == "NF") # Let put NF from h to notes
...
november_surv %>%
  filter(h != "NF", e == "NF")
november_surv[which(november_surv$h != "NF" & november_surv$e == "NF"), "h"] <-
  "NF"
...
jan <- january_surv2 %>%
  select(ter, plot, label, ms, com, species, watering)
mar <- mar_surv2 %>%
  select(ter, plot, label, ms, com, species, watering)
jun <- june_surv2 %>%
  select(ter, plot, label, ms, com, species, watering)
...
### 1081
filter(january_surv2, label == 1081)
# 1081 watering column is badly reported as 0
...
### 804
january_surv2 %>% filter(label == 804)
# In january, 804 ms is badly recorded as 0
mar_surv2 %>% filter(label == 804) june_surv2 %>% filter(label == 804)
...
january_surv2 %>% filter(label == 1013) mar_surv2 %>% filter(label == 1013)
june_surv2 %>% filter(label == 1013) november_surv2 %>% filter(label == 1013)
...
# Ok,not present in them
mar_surv2[which(mar_surv2$label == 1013 & mar_surv2$plot
  == 10), "label"] <- rep(1113, 3)
november_surv2[which(november_surv2$label == 1013 & november_surv2$plot == 10),
  "label"] <- rep(1113, 3)
...
if(january_surv2 %>% filter( label == 857) %>% nrow(.) == 0){
...
### 579
january_surv2 %>% filter(label == 579) mar_surv2 %>% filter(label == 579)
june_surv2 %>% filter(label == 579) november_surv2 %>% filter(label == 579)
#it is missing in november
#### May be recorded as 578 in november
november_surv2 %>% filter(label == 578)
# It is the case
june_surv2 %>%
  filter(label == 578)
...
try2 <- try1 %>%
  mutate(
    date = str_replace_all(date, c("jan" = "2016-01", "mar" = "2016-03", "jun" =
        "2016-06", "nov" = "2016-11")),
    date = as.yearmon(date), month = month(date),
...
temp <-
  filter(january, label == 911) %>%
  mutate(long_nurse = NA, larg_nurse
    = NA, nurse_dead = NA) %>%
...
if(filter(november, label == 803) %>% nrow(.) == 0){
  temp2 <- mutate(temp, date = "nov") november <- rbind(november, temp2)
}
...
## Who's bad ?
filter(label_characteristics, nurse_dead %in% c("bad_nurse", "very_bad_nurse"))
#12 in bad
# 6 very bad
#### Change ?
label_characteristics2 <- mutate(
  label_characteristics,
...
try_survival <- try2 %>%
  ...
  summarise( surv = mean(survival, na.rm = TRUE),
