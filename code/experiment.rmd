...
We will measure functional traits during the month of march. We will measure Specific Leaf Area, $\delta C^{13}$, leaf height and leaf width.
We will collect one leaf by individual. Each leaf will be stored in Falcon tubes filled humid (deionized water) piece of soft paper.
Those falcon tubes will be stored in glacial boxes.

We collected only leaves in good shape, mature and the most exposed to the sunbeam. Furthermore, we collect only on the individuals which had sufficient leaves to not suffer of the sampling (subjective point of view).

We collected 3 leaves by Pistachia, 5 leaves by Anthyllis and 10 leaves by Dorycnium by respect to the weight of species leaves.

### Lab

When the leaves were taken from the tubes, we first rehydrated them with deionized water during several minutes. Following this, we weighted them, scanned, measured them, put them in an envelope, and finally put them in the drier at least 48h at 72 celcius degrees. 
...
data <- load.traits(data="~/Documents/thesis/alicante/thesis_chap2_alicante/data/data_mars_traits.csv")
...
```{r load_data_march, echo = FALSE, fig.height=2, fig.width=2}

data.january <- LoadPlant()
data.march <- LoadPlantMarch()

m <- interaction(data.march[,"label"], data.march[,"sp"], sep="-")
j <- interaction(data.january[,"label"], data.january[,"sp"], sep="-")


growth.d <- data.march[m %in% j, "d"] - data.january[m %in% j, "d"]
hist(growth.d)

growth.h <- data.march[m %in% j, "h"] - data.january[m %in% j, "h"]
hist(growth.h)

data.growth <- cbind(data.january[,c("ter", "plot", "label","ms", "com", "watering", "sp", "ind")], 
	growth.d, growth.h)

```

## Growth January March

### Diameter

```{r growth_d, echo = FALSE, fig.width=8, fig.height=8}
normal <- par()
par(mfrow=c(2,2))
microsite.stat <- data.growth[data.growth$watering==0 & data.growth$com!="M",]
plotmeans(growth.d~ interaction(ms, com, sep="|"),
       	data=microsite.stat,
...
abline(0,0,col="red")
microsite.stat <- data.growth[data.growth$watering==1 & data.growth$com!="M",]
plotmeans(growth.d~ interaction(ms, com, sep="|"),
       	data=microsite.stat,
	connect=list(1:2,3:4,5:6),
	n.label=F,
...
abline(0,0,col="red")
microsite.stat <- data.growth[data.growth$watering==0 & data.growth$com=="M",]
plotmeans(growth.d~ interaction(ms, sp, sep="|"),
       	data=microsite.stat,
	connect=list(1:2,3:4,5:6),
# 	n.label=F,
 	ylim=c(-0.6,1.25),
...
```{r growth_h, echo = FALSE, fig.width=8, fig.height=8}
par(mfrow=c(2,2))
microsite.stat <- data.growth[data.growth$watering==0 & data.growth$com!="M",]
plotmeans(growth.h~ interaction(ms, com, sep="|"),
       	data=microsite.stat,
	connect=list(1:2,3:4,5:6),
	n.label=F,
	ylim=c(-7,7),
...
abline(0,0,col="red")
microsite.stat <- data.growth[data.growth$watering==0 & data.growth$com=="M",]
plotmeans(growth.h~ interaction(ms, sp, sep="|"),
       	data=microsite.stat,
	connect=list(1:2,3:4,5:6),
# 	n.label=F,
	ylim=c(-7,7),
...
```{r ldmc_ms, echo = FALSE, fig.width=9, fig.height=8}
oldw <- getOption("warn")
options(warn = -1)
...
The results show that the LDMC of Dorycnium and Anthyllis has shifted toward lower value, that means, a more acquisitive strategy (versus conservative). It's nice because it was predicted by the theory.
