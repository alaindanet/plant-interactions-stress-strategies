...
```{r allometry, echo = FALSE , fig.cap="Relationship between total biomass part and basal diameter (on the left), and height (on the right). Black: Dorycnium, green: Pistachia, red: Anthyllis", fig.width=7}
...
mod <- lm(ldmc~ms+watering+sp , data[data$com!="M",])
```

#### Community effect

```{r ldmc_community_open, echo = FALSE}

microsite.stat <- data[data$watering==0 & data$ms!=0,]

plotmeans(ldmc~ interaction( com, sp, sep="|"),
       	data=microsite.stat,
	connect=list(1:2,3:4,5:6)	)
...
```{r ldmc_community_patch, echo = FALSE}

microsite.stat <- data[data$watering==0 & data$ms!=1,]
...
mod <- lm(ldmc~ms+watering+com , data)
```
